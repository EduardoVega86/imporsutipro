...
class WalletModel extends Query
{
    ...
    public function abonarBilletera($id_cabecera, $valor, $usuario)
    {
        ...
        if ($proveedor != NULL && $proveedor != $tienda) {
            ...
            $sql = "INSERT INTO cabecera_cuenta_pagar (`tienda`, `numero_factura`, `guia`, `costo`, `monto_recibir`, `valor_pendiente`, `estado_guia`, `visto`, `full`, `fecha`, `cliente`, `id_plataforma`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $response =  $this->insert($sql, array($proveedor, $numero_factura . '-P', $guia, $costo, $costo - $full, $costo - $full, 7, 0, $full, $fecha, $cliente, $id_plataforma));
        }
