...
class DashboardModel extends Query
{
    ...
    public function filtroInicial($fecha_i, $fecha_f, $plataforma, $id_plataforma)
    {
     ...
     estado_descripcion,
        ...
        $sql = "SELECT ct.ciudad, COUNT(fc.ciudad_cot) AS cantidad_entregas FROM facturas_cot fc INNER JOIN ciudad_cotizacion ct ON fc.ciudad_cot = ct.id_cotizacion WHERE fc.estado_guia_sistema IN (9, 500, 501, 502, 503) AND fc.id_plataforma = $id_plataforma   AND fecha_guia BETWEEN '$fecha_i' AND '$fecha_f'  GROUP BY ct.ciudad ORDER BY cantidad_entregas DESC LIMIT 5;";
