...
class Guias extends Controller
{
    ...
    public function generarGintracom()
    {
        ...
        if (isset($response["guia"])) {
            $response["status"] = 200;
            $this->model->aumentarMatriz();
            $response2 = $this->model->actualizarGuia($numero_factura, $response["guia"], $ciudad, $ciudadDestino, $direccionDestino, $telefonoDestino, $celularDestino, $referenciaDestino, $cod, $costo_producto, $comentario, $_SESSION["id"], $_POST['calle_principal'], $_POST['calle_secundaria'], $contiene, $provincia, $costoflete, "GINTRACOM");
