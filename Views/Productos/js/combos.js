...
document.addEventListener("DOMContentLoaded", () => {
    ...
    $("#editar_comboModal").on("shown.bs.modal", function() {
        const id_combo = $(this).data('id'); // Obtener el id del combo desde un atributo de datos
        if (id_combo) {
            editar_combo(id_combo); // Llamar a la función de edición con el id del combo
        }
    });

    function fetchProductos(productoId = null) { // Agregamos el parámetro productoId
    ...
    function editar_combo(id_combo) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // Cargar los productos y luego seleccionar el producto adecuado
