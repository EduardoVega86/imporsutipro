...
let dataTableSolicitudesReferidos;
let dataTableSolicitudesReferidosIsInitialized = false;

const dataTableSolicitudesReferidosOptions = {
  columnDefs: [
    { className: "centered", targets: [1, 2, 3, 4, 5] },
    { orderable: false, targets: 0 }, //ocultar para columna 0 el ordenar columna
  ],
...
const initDataTableSolicitudesReferidos = async () => {
  if (dataTableSolicitudesReferidosIsInitialized) {
    dataTableSolicitudesReferidos.destroy();
  }

  await listSolicitudesReferidos();

  dataTableSolicitudesReferidos = $(
    "#datatable_solicitudes_referidos"
  ).DataTable(dataTableSolicitudesReferidosOptions);

  dataTableSolicitudesReferidosIsInitialized = true;
};
...
const listSolicitudesReferidos = async () => {
  try {
    ...
    solicitudesReferidos.forEach((solicitud, index) => {
      content += `
      <tr>
      <td><input type="checkbox" class="selectCheckbox" data-id="${solicitud.id_solicitud}" ${checkboxState} onclick="toggleSolicitud(${solicitud.id_solicitud}, this.checked)"></td>
      <td>${nombre_tienda}</td>
    ...
    document.getElementById("tableBody_solicitudes_referidos").innerHTML =
      content;
  } catch (ex) {
    alert(ex);
  }
...
window.addEventListener("load", async () => {
  await initDataTableSolicitudesReferidos();
});
