...
function removeDynamicSelects(parentSelectId, isPrincipal) {
  ...
  const dynamicSelects = parentSelect.parentNode.querySelectorAll(
    `select[id^='${parentSelectId}-dynamic']`
  );
  ...
  if (isPrincipal) {
    const allDynamicSelects = parentSelect.parentNode.querySelectorAll(
      `select[id*='-dynamic']`
    );
